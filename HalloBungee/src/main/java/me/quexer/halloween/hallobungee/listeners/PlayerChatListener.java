...
public class PlayerChatListener implements Listener {
    ...
    public PlayerChatListener(HalloBungee plugin) {
        this.plugin = plugin;
    }
     ...
     */
        if (!event.getMessage().startsWith("/")) {
            if (plugin.getBackendPlayer(((ProxiedPlayer) event.getSender()).getUniqueId().toString()).getMutePlayer().isPunished()) {
                BackendPlayer backendPlayer = plugin.getBackendPlayer(((ProxiedPlayer) event.getSender()).getUniqueId().toString());
                if (System.currentTimeMillis() < backendPlayer.getMutePlayer().getEnd() || backendPlayer.getMutePlayer().getEnd() == -1) {
                    ...
                    proxiedPlayer.sendMessage(plugin.getBanPrefix());
                    proxiedPlayer.sendMessage(plugin.getBanPrefix()+"§7Gemutet von§8: §e"+plugin.getGroup(backendPlayer.getMutePlayer().getPunished_from()).getPrefix()+ UUIDFetcher.getName(UUID.fromString(backendPlayer.getMutePlayer().getPunished_from())));
